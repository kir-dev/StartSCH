...
public class CmschPollJob(CmschModule cmschModule, HttpClient httpClient, Db db) : IPollJobExecutor
{
    public async Task Execute(CancellationToken cancellationToken)
    {
        foreach (string frontendUrl in cmschModule.Instances)
        {
            var indexHtmlStream = await httpClient.GetStreamAsync(frontendUrl, cancellationToken);
            ...
            var app = await httpClient.GetFromJsonAsync<AppResponse>($"{backendUrl}/api/app", cancellationToken);
            ...
            if (defaultCategory.Id != 0)
                await db.Events.Include(e => e.EventCategories).Where(e => e.Categories.Contains(defaultCategory)).LoadAsync(cancellationToken);

            Event? currentEvent = defaultCategory.Events.FirstOrDefault();
            
            // assume new event after 2.5 months of no activity
            if (currentEvent != null)
            {
                DateTime lastPost = await db.Posts
                    .OrderByDescending(p => p.Updated)
                    .Select(p => p.Updated)
                    .FirstOrDefaultAsync(cancellationToken);
                DateTime lastEvent = await db.Events
                    .OrderByDescending(p => p.Updated)
                    .Select(p => p.Updated)
                    .FirstOrDefaultAsync(cancellationToken);
                DateTime? lastUpdate = lastPost != default && lastEvent != default
                    ? (lastPost > lastEvent ? lastPost : lastEvent)
                    : (lastPost != default ? lastPost : lastEvent);
                if (DateTime.UtcNow - lastUpdate > TimeSpan.FromDays(75))
                    currentEvent = null;
            }
            ...
            {
                EventsView eventsView = (await httpClient.GetFromJsonAsync<EventsView>($"{backendUrl}/api/events", cancellationToken))!;
            ...
            {
                NewsView newsView = (await httpClient.GetFromJsonAsync<NewsView>($"{backendUrl}/api/news", cancellationToken))!;
